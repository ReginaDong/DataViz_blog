...
![](to be updated) 
_Fig.3 Sketch of alternative design_
...
2      |Stack the data frame| Add a column called "Country", and stack the data frame using copy and paste, re-saved in a new Excel workbook named "data_stacked_cleaned". The column contains export trade amount was named as "Export", and the column contains import trade amount named "Import", after stacking the data should be like the following pictures shows: <br> ![](img/step2.jpg) <br> _Fig.5 Data frame after stacking_
3      |Import data into Tableau| Launch tableau -> Connect to a file "Microsoft Excel" -> Select "data_stacked_cleaned", as there is only one sheet in the workbook, it will be automatically connected. Keep the Connection type be "Live" 
...
Below list visualization steps for Worksheet:
...
15     |Add Annotations for the highest and lowest quarterly total amount of export and import points by right click on the target points -> Annotate -> Mark..., set the font of annotation as 8, and input in the Edit Annotation pane as following accordingly: <br>  ```Maximum quarterly export amount: <SUM(Export)>``` <br> ```Maximum quarterly import amount: <SUM(Import)>``` <br> ```Minimum quarterly export amount: <SUM(Export)>``` <br> ```Minimum quarterly import amount: <SUM(Import)>``` <br> Then drag the annotations to the proper position as right side figure shows| ![](img/step2.17.jpg){width=100%} <br> _Fig.21 Create annotations_

Below list visualization steps for Dashboard:

**NO.**|**Step**              |**Action**
-------|----------------------|--------------------------------------------
16     |Click the New Dashboard button and drag Sheet 1 into blank dashboard, then change the size of Dashboard as Generic Desktop (1366x768). Right click on title area on the top and select "Hide Title"| ![](img/step2.18.jpg){width=100%} <br> _Fig.22 Add Dashboard 1_
...
18     |From Objects pane, click on Text and drag to area just below graph title, and type in words like right side figure shows. Keep the defaul font settings| ![](img/step2.20.jpg){width=80%} <br> _Fig.24 Edit lead-in verbiage_ 
