...
Set *day* as filtering criteria, and differentiate the line colors by *id*, so that the geographical graph could show the movement track of all cars in specific date.  

```{r}
gps_path_selected <- gps_path2 %>%
  filter(day=="6") 

#gps_path_selected$id = as.numeric(gps_path_selected$id)
...
tm_shape(bgmap) +
  ...
  tm_layout(title = "GPS Moving Route of A Specific Day")
 
```


Set *hour* as filtering criteria, and differentiate the line colors by *day*, so that the geographical graph could show the movement track of all cars in specific hour of days.  
...
tm_shape(bgmap) +
  ...
  tm_lines(col = "id", palette = "Dark2") +
  tmap_options(max.categories = 44) +
  tm_layout(title = "GPS Moving Route of A Specific Hour")
 
```

## 3.3 Network visualization according to movement route
