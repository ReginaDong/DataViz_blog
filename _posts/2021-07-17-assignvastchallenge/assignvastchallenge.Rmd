...
crosstalk::bscols(gra_1,
                  crosstalk::filter_select("day", "Day", d, ~as.factor(day), multiple = T), widths = 10)
```
...
Then before combining the background map and the GPS tracking lines to generate the movement path, the spatial data need to be grouped by *id*, *day* and *hour* respectivly.
...
Set *day* as filtering criteria, and differentiate the line colors by *id*, so that the geographical graph could show the the movement track of all cars in specific date.  
